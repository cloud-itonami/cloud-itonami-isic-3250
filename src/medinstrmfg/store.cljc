(ns medinstrmfg.store
  "SSoT for the medical-and-dental-instrument plant-operations
  ...
  self-contained (no external medical-instrument-manufacturing
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             device-class/sterility-assurance-level/
                             quantity/nonconformance-rate record.
    ...
    - `equipment`         -- a machining/molding/sterilization unit's
                             ...
                             `batches`.
    - `maintenance`       -- a scheduled machining/molding/
                             sterilization-equipment maintenance-window
                             DRAFT against a piece of equipment
                             (`medinstrmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound medical/dental-
                             instrument shipment DRAFT
                             (`medinstrmfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (medinstrmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (medinstrmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :device-class :class-ii
                :lot-number "MDL-2026-001" :sterility-assurance-level 6
                :quantity-units 5000.0 :nonconformance-rate-percent 0.8
                :verified? true :registered? true
                :shipped-units 1000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :device-class :class-iii
                :lot-number "MDL-2026-002" :sterility-assurance-level 6
                :quantity-units 800.0 :nonconformance-rate-percent 0.3
                :verified? true :registered? true
                :shipped-units 750.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :device-class :class-i
                :lot-number "MDL-2026-003" :sterility-assurance-level 6
                :quantity-units 6000.0 :nonconformance-rate-percent 2.3
                :verified? false :registered? false
                :shipped-units 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"machining-001" {:id "machining-001" :kind :machining-center
                     :verified? true :registered? true
                     :last-maintenance-date "2026-05-01"}
   "sterilizer-002" {:id "sterilizer-002" :kind :sterilizer
                      :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `medinstrmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `medinstrmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  machining-center unit (schedulable for maintenance), one
  UNVERIFIED/unregistered sterilizer unit (blocks any maintenance
