(ns medinstrmfg.operation
  "MedInstrOperationActor -- one plant-operations coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (MedInstrAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Medical
  ...
  (:require [langgraph.graph :as g]
            ...
            [medinstrmfg.advisor :as advisor]
...
(defn build
  "Compiles a MedInstrOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `medinstrmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; MedInstrAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Medical Instrument Plant Operations Governor -- independent censor.
      (g/add-node :govern
