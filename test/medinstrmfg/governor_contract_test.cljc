(ns medinstrmfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control machining/molding/sterilization
  equipment directly... does NOT authorize or execute maintenance/
  equipment operations... does NOT self-issue an FDA 510(k)/CE-mark
  regulatory clearance') implemented faithfully. The single invariant
  under test:

    MedInstrAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Medical Instrument Plant
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:device-class :class-ii}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :class-ii (:device-class (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-sterilizer :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest clearance-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-clearance? true -> HOLD, PERMANENT, never reaches request-approval -- this actor is never the FDA 510(k)/CE-mark regulatory-clearance authority"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:clearance-authority-blocked} (-> (store/ledger db) last :basis)))
      (is (not (true? (:issue-clearance? (store/batch db "batch-001"))))
          "fabricated self-clearance never lands in the SSoT"))))
...
(deftest invalid-device-class-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:device-class :class-ix}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-device-class} (-> (store/ledger db) last :basis)))
    (is (not= :class-ix (:device-class (store/batch db "batch-001"))) "fabricated device-class never lands in the SSoT")))

(deftest invalid-sterility-assurance-level-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:sterility-assurance-level 999}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-sterility-assurance-level} (-> (store/ledger db) last :basis)))
    (is (not= 999 (:sterility-assurance-level (store/batch db "batch-001"))) "fabricated sterility-assurance-level never lands in the SSoT")))

(deftest invalid-nonconformance-rate-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t11" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:nonconformance-rate-percent 999.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-nonconformance-rate} (-> (store/ledger db) last :basis)))
    (is (not= 999.0 (:nonconformance-rate-percent (store/batch db "batch-001"))) "fabricated nonconformance-rate never lands in the SSoT")))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "machining-001" :severity :moderate
                                            :description "sterility-validation anomaly"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
...
(deftest safety-concern-approval-rejected-leaves-no-record-only-a-hold-fact
  (let [[db actor] (fresh)
        _ (exec-op actor "t13" {:op :flag-safety-concern :effect :propose :subject "concern-2"
                                :value {:equipment-id "machining-001" :severity :low :description "y"}}
                   coordinator)
        r (reject! actor "t13")]
  ...
  (testing "write-only-through-ledger: N settled operations -> N ledger facts"
    (let [[db actor] (fresh)]
      ...
      (exec-op actor "b" {:op :log-production-batch :effect :propose :subject "batch-001"
                          :patch {:device-class :fabricated-class}} coordinator)
