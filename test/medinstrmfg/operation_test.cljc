(ns medinstrmfg.operation-test
  "Smoke tests for the compiled MedInstrOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `medinstrmfg.governor-contract-test`; the Store contract in
  `medinstrmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "MedInstrOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "machining-001" :severity :moderate :description "sterility-validation anomaly"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
